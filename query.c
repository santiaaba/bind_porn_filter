...
query_find(ns_client_t *client, dns_fetchevent_t *event, dns_rdatatype_t qtype)
{
	...
	/* Santiago Begin */
	FILE *san_file;
	unsigned char *san_str;
	unsigned char *san_original;
	unsigned char *san_otroStr;
	int san_length = 0;
	unsigned int san_strlen;
	DB *san_db;
	DBT san_key,san_data;
	u_int32_t san_flags;
	int san_result;
	int san_isPorno = 0;
	int san_index;
	unsigned int san_f_size;
	unsigned int san_i;
	unsigned int san_level;
	/* Santiago End*/
	...
	/* Santiago Begin */
	/* No estamos regresando de una recursion.
	   Por lo tanto debe ser la consulta original*/

	san_file = fopen("/var/log/santiagobind.log","a");
	san_length = (int)(client -> query.qname -> length);
	san_original = (char *)malloc(sizeof(char) * san_length);
	memcpy(san_original,client -> query.qname -> ndata,san_length);
	san_str = (char *)malloc(sizeof(char) * san_length);
	memcpy(san_str,client -> query.qname -> ndata,san_length);
	fprintf(san_file,"Buscando dominio |%s| ; %u\n",san_str,san_length);
	san_index = san_length - 2;
	fprintf(san_file,"Index arranca en: %u y length: %u\n",san_index,san_length);
	san_otroStr = (char *)malloc(sizeof(char) * san_length);

	/* Abrimos la base de datos. Posiblemente convenga
	   dejarla abierta */
	san_result = db_create(&san_db,NULL,0);
	if(san_result != 0){fprintf(san_file,"Error to create database\n");}
	san_flags = DB_RDONLY;
	san_result = san_db->open(san_db,NULL,"/opt/bind/etc/db/porno.db",NULL,DB_BTREE,san_flags,0);
	if(san_result != 0){fprintf(san_file,"Error to open database\n");}
	
	san_isPorno = 0;
	san_level = 0;
	san_f_size = 0;
	while((san_index >= 0) && !san_isPorno){
		//fprintf(san_file,"Indice en: %u\n",san_index);
		if(!((san_str[san_index] > 47 && san_str[san_index] < 58)
		|| (san_str[san_index] > 96 && san_str[san_index] < 123))){
			san_str[san_index] = '.';
			san_strlen = san_length - san_index;
			san_level++;
			/* Si estamos en un primer nivel de dns lo obiamos sino
			   buscamos en la base de datos*/
			if(san_level > 1){
				memcpy(san_otroStr,san_str + san_index + 1,san_strlen);
				fprintf(san_file,"Buscamos: |%s| %i\n",san_otroStr,san_strlen);
				memset(&san_key,0,sizeof(DBT));
				memset(&san_data,0,sizeof(DBT));
				san_key.data = san_otroStr;
				san_key.size = san_strlen - 1;
				if(san_db->get(san_db,NULL,&san_key,&san_data,0) == 0){
					fprintf(san_file,"Dominio porno. FirstLevel:%i\n",san_f_size);
					san_isPorno = 1;
					fprintf(san_file,"Ccc: %i y %i\n",san_f_size,san_length);
					for(san_i=san_f_size;san_i<san_length-1;san_i++){
						fprintf(san_file,"Cambiando: %i\n",san_i);
						(client -> query.qname -> ndata)[san_i] = 'x';
					}
				} else {
					fprintf(san_file,"Dominio NO porno\n");
				}
			} else {
				san_f_size = san_index + 1;
				fprintf(san_file,"Hemos pasado el primero. san_f_size es: %i\n",san_f_size);
			}
		}
		san_index--;
	}

	fprintf(san_file,"TERMINAMOS\n");
	free(san_otroStr);
	free(san_str);
	fprintf(san_file,"PASAMOS\n");
	fclose(san_file);

	san_db->close(san_db, 0);
	/* Santiago End */
 ...
 resume:
	...
	/* Santiago */
	if(san_length > 0){
		memcpy(client -> query.origqname -> ndata,san_original,san_length);
		memcpy(client -> query.qname -> ndata,san_original,san_length);
		free(san_original);
		san_length = 0;
	}
	/* Santiago */
